<template>
  <div>
    <dol-list
      ...
      <el-table-column prop="imgUrl" label="话题缩略图">
        <template slot-scope="scope">
          <img :src="$Dolphin.path.publicPath+scope.row.imgUrl" class="activityImg" alt="">
        </template>
      </el-table-column>
      <el-table-column prop="introduction" label="话题简介"/>
...
<script>
  ...
  export default {
    data() {
      return {
        url: {
          listTopic: '/api/admin/topic/list',
          saveTopic: '/api/admin/topic/save/{id}',
          removeTopic: '/api/admin/topic/remove/{id}',
        },
        queryForm: {},
        detailForm: {},
        uniformFile: false,
